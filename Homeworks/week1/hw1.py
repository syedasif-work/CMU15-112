...
def nearestBusStop(street):
    return 8 * roundHalfUp((street-1) / 8)
...
def setKthDigit(n, k, d):
    ...
    hundreds = n // 100 % 10
    if k == 0:
        result =  hundreds * 100 + tens * 10 + d
    elif k == 1: 
        result = hundreds * 100 + d * 10 + ones
    elif k == 2:
        result =  d * 100 + tens * 10 + ones
    else:
        result = d* 10 ** k + hundreds * 100 + tens * 10 + ones
    if temp < 0:
        return -result
    else: 
        return result
